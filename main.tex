...
Define the two-input polynomial map
\[
T:\R^3\times\R\longrightarrow\R^3
\]
by
\[
T(v,s)=H(v)-(1-s)\Delta.
...
For $s\in\{0,1\}$,
\[
\exists v\in\R^3:\ T(v,s)=0
\quad\Longleftrightarrow\quad
s=1.
\]
Moreover,
\[
\det D_vT(v,s)\equiv -3456.
\]
\end{lemma}

\begin{proof}
If $s=1$, then $T(v,1)=H(v)$, so for example $T(r_0,1)=0$.  If $s=0$, then
$T(v,0)=0$ would imply $H(v)=\Delta$, which is impossible by the fixed-gadget
